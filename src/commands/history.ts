...
const executeHistory = async (client: Client<boolean>, interaction: ChatInputCommandInteraction<CacheType>) => {
  ...
  else if (subcommand === 'personal')
    await executeHistoryQueue(client, interaction, embed);
}

const executeHistoryPlay = async (client: Client<boolean>, interaction: ChatInputCommandInteraction<CacheType>, embed: EmbedBuilder) => {
  ...
  embed.setTitle('History')
    .setDescription(`History of music played (${resultsPerPage} items per page)`);
...
const executeHistoryQueue = async (client: Client<boolean>, interaction: ChatInputCommandInteraction<CacheType>, embed: EmbedBuilder) => {
